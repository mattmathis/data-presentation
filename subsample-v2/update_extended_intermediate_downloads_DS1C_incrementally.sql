...
# DROP TABLE  `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C`;

# Reset the tmp table - only needed after a schema change
# DROP TABLE  `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C_tmp`; 

/* FIRST TIME ONLY

CREATE OR REPLACE TABLE `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C` 
  PARTITION BY date
...
SELECT * FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C`
WHERE date = '2008-01-01' -- Out of range
  UNION ALL
( SELECT * FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS16`
  WHERE date = '2008-01-01' -- Out of range
);
...
CREATE OR REPLACE TABLE `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C_tmp` 
  PARTITION BY date
  CLUSTER BY isValidBest, ServerContinent, ServerSite
  OPTIONS (require_partition_filter = true)
AS ( SELECT *,
...
DELETE FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C` WHERE date = replace_day; -- Usually a noop

-- insert the new data into the same partition in mytable
INSERT INTO `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C` 
  SELECT * FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C_tmp`
  WHERE date = replace_day
;
