# Backfill_scamper2_DS16_incrementally
#
# DML script — works BACKWARDS from the oldest existing partition to stop_date.
# Adjust stop_date before running.

# Monitor progress:
# SELECT MIN(date) FROM `mlab-collaboration.mm_preproduction.scamper2_DS16` WHERE date >= '2026-01-01'
...
SET replace_day = (SELECT MIN(date) FROM `mlab-collaboration.mm_preproduction.scamper2_DS16` WHERE date >= stop_date);
